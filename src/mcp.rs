...
fn tool_add_contact(ctx: &McpContext, args: &Value) -> Value {
    let name = match args.get("name").and_then(|v| v.as_str()) {
        Some(n) => n.to_string(),
        None => return json!({"content": [{"type": "text", "text": "Missing 'name'"}], "isError": true}),
    };
    let address = match args.get("address").and_then(|v| v.as_str()) {
        Some(a) => a.to_string(),
        None => return json!({"content": [{"type": "text", "text": "Missing 'address'"}], "isError": true}),
    };
    let psk_str = match args.get("psk").and_then(|v| v.as_str()) {
        Some(p) => p.to_string(),
        None => return json!({"content": [{"type": "text", "text": "Missing 'psk'"}], "isError": true}),
    };
    ...
    let psk_bytes = match crate::contacts::parse_psk(&psk_str) {
        Ok(b) => b,
        Err(e) => return json!({"content": [{"type": "text", "text": format!("Invalid PSK: {}", e)}], "isError": true}),
    };
    ...
    match ContactStore::open(&path) {
        Ok(store) => match store.add(&name, &address, &psk_bytes) {
            ...
            Err(e) => json!({"content": [{"type": "text", "text": format!("Failed to add: {}", e)}], "isError": true}),
        },
        Err(e) => json!({"content": [{"type": "text", "text": format!("Failed to open contacts: {}", e)}], "isError": true}),
    }
}
...
fn tool_remove_contact(ctx: &McpContext, args: &Value) -> Value {
    ...
    match ContactStore::open(&path) {
        Ok(store) => match store.remove(&name) {
            Ok(true) => json!({"content": [{"type": "text", "text": format!("Removed contact: {}", name)}]}),
            Ok(false) => json!({"content": [{"type": "text", "text": format!("Contact '{}' not found", name)}]}),
...
fn tool_list_messages(ctx: &McpContext, args: &Value) -> Value {
    ...
    let from_filter = args.get("from").and_then(|v| v.as_str()).map(|s| s.to_string());
    ...
    let conn = match rusqlite::Connection::open(&messages_db) {
        Ok(c) => c,
        Err(e) => return json!({"content": [{"type": "text", "text": format!("Failed to open messages.db: {}", e)}], "isError": true}),
    };
    ...
    let rows_result: rusqlite::Result<Vec<(String, String, i64, String)>> = if let Some(ref addr) = from_address {
        let mut stmt = match conn.prepare(query) {
            Ok(s) => s,
            Err(e) => return json!({"content": [{"type": "text", "text": format!("Query error: {}", e)}], "isError": true}),
        };
    ...
    match rows_result {
        Ok(rows) if rows.is_empty() => json!({"content": [{"type": "text", "text": "No messages found."}]}),
        Ok(rows) => {
            let list: Vec<Value> = rows
                .iter()
                .map(|(sender, content, ts, dir)| json!({
                    "from": sender,
                    "content": content,
                    "timestamp": ts,
                    "direction": dir,
        ...
        Err(e) => json!({"content": [{"type": "text", "text": format!("Query failed: {}", e)}], "isError": true}),
    }
}
...
async fn tool_send_message(ctx: &McpContext, args: &Value) -> Value {
    let to = match args.get("to").and_then(|v| v.as_str()) {
        Some(t) => t.to_string(),
        None => return json!({"content": [{"type": "text", "text": "Missing 'to'"}], "isError": true}),
    };
    let message = match args.get("message").and_then(|v| v.as_str()) {
        Some(m) => m.to_string(),
        None => return json!({"content": [{"type": "text", "text": "Missing 'message'"}], "isError": true}),
    };
    ...
    let key_storage = match SqliteKeyStorage::open(data_path.join("keys.db")) {
        Ok(s) => s,
        Err(e) => return json!({"content": [{"type": "text", "text": format!("Failed to open key storage: {}", e)}], "isError": true}),
    };
    let message_cache = match SqliteMessageCache::open(data_path.join("messages.db")) {
        Ok(c) => c,
        Err(e) => return json!({"content": [{"type": "text", "text": format!("Failed to open message cache: {}", e)}], "isError": true}),
    };
    ...
    {
        Ok(c) => c,
        Err(e) => return json!({"content": [{"type": "text", "text": format!("Failed to initialize AlgoChat: {}", e)}], "isError": true}),
    };
        ...
        if let Ok(store) = ContactStore::open(&contacts_path) {
            if let Ok(contacts) = store.list() {
                for contact in &contacts {
                    ...
                    let _ = client.add_psk_contact(&contact.address, &psk, Some(contact.name.clone())).await;
                }
            }
        }
    }
    ...
    .await
    {
        ...
        Err(e) => json!({"content": [{"type": "text", "text": format!("Failed to send: {}", e)}], "isError": true}),
    }
}
